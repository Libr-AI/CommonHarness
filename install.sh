...
#
# Layout produced:
#   ~/.commonharness/v0.7.2/        ← pinned snapshot (one dir per version)
#   ~/.commonharness/v0.8.0/
#   ~/.commonharness/current  →  v0.8.0   ← which one is "active"
...
VERSION="${HARNESS_VERSION:-v0.8.0}"
...
Upgrade later (example — replace v0.8.0 with the next release):

  HARNESS_VERSION=v0.8.0 \$HOME/.commonharness/current/install.sh
