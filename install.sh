...
#
# Layout produced:
#   ~/.commonharness/v0.7.1/        ← pinned snapshot (one dir per version)
#   ~/.commonharness/v0.7.2/
#   ~/.commonharness/current  →  v0.7.2   ← which one is "active"
...
VERSION="${HARNESS_VERSION:-v0.7.2}"
...
Upgrade later (example — replace v0.7.2 with the next release):

  HARNESS_VERSION=v0.7.2 \$HOME/.commonharness/current/install.sh
